...
#include <ctype.h>
#include <time.h> /* DAL */
#include <sys/stat.h> /* DAL */

#include "nasm.h"
#include "nasmlib.h"
#include "outform.h"
#include "cvinfo.h" /* DAL */

#ifdef OF_OBJ
extern struct IncPath
{
    struct IncPath *next;
    char *path;
} *ipath ;
...
enum RecordID {			       /* record ID codes */
    ...
    LINN32 = 0x95,             /* DAL line number, 32 bit */
...
enum ComentID {                        /* ID codes for comment records */
     ...
     dDEPNAME = 0xe9,                  /* Dependency name record */
...
static enum { dbg_none, dbg_bi, dbg_ls } dbg_type = dbg_none; /* DAL, debug info mode */
/*
...
/* DAL */
   if (val & 0xc0000) {
      val &= ~0xc0000 ;
      return obj_word(orp, ((val >> 8) | 0xc0) | (val << 8) ) ;
   }
/* end DAL */
    if (val < 128)
...
/* start DAL */
static ObjRecord * obj_filetime(ObjRecord *orp, char *name)
{
    struct stat data ;
    if (!stat(name,&data)) {
        struct tm tmx = *localtime(&data.st_mtime) ;
        unsigned short timex = (tmx.tm_hour << 11) + (tmx.tm_min << 5) + tmx.tm_sec /2 ;
        unsigned short date = ((tmx.tm_year + 1900 - 1980) << 9) + ((tmx.tm_mon +1)<< 5) + tmx.tm_mday ;
        obj_word(orp,timex) ;
        obj_word(orp,date) ;
    } else
        obj_dword(orp,0) ;
...
/* END DAL */
    ...
    if (fnhead && debuginfo) {
        ...
        /* Begin DAL */
            if (dbg_type == dbg_ls && seg->lsdebseg != 1)
                continue ;
        /* End DAL */
		orp->parm[0] = seg->grp ? seg->grp->obj_index : 0;
		orp->parm[1] = seg->obj_index;
     ...
     */
    if (debuginfo && dbg_type == dbg_bi && obj_entry_seg != NO_SEG) { /* DAL*/
     ...
     */
    for (seg = seghead; seg && debuginfo && dbg_type == dbg_bi; seg = seg->next) { /* DAL */
        /* labels this seg */
...
//    if (!nasm_stricmp(fn->name,lnfname))
//        break;
        char * p = stristr(fn->name,lnfname) ;
        if (p && (p == fn->name || p[-1] == '\\' || p[-1] == '/'))
            break ;
        /* end DAL */
    }
    if (!fn) {
        ...
        FILE *f = 0;
        if (dbg_type == dbg_ls) {
            char *p = stristr(obj_infile,lnfname) ;
            if (!p || (p[-1] != '\\' && p[-1] != '/')) {
                strcpy(buf,lnfname) ;
                if (!(f = fopen(buf,"rb"))) {
                    struct IncPath *path = ipath ;
                    while (path) {
                ...
                strcpy(buf,obj_infile) ;
        } else
            strcpy(buf,lnfname) ;
        /* end DAL  */
	fn = nasm_malloc ( sizeof( *fn));
    fn->name = nasm_malloc ( strlen(buf) + 1) ; /* DAL */
        strcpy (fn->name,buf); /* DAL */
	fn->lnhead = NULL;
	fn->lntail = & fn->lnhead;
	fn->next = NULL;
	*fntail = fn;
...
/* dbgls functions by DAL */
static void dbgls_init(struct ofmt * of, void * id, FILE * fp, efunc error)
{
    (void) of;
    (void) id;
    (void) fp;
...
static void dbgls_typevalue (long type)
{
    ...
    switch (type) {
	case TY_BYTE:
        last_defined->type = T_UCHAR; /* unsigned char */
	    vsize = 1;
	    break;
	case TY_WORD:
        last_defined->type = T_USHORT; /* unsigned word */
	    vsize = 2;
	    break;
	case TY_DWORD:
        last_defined->type = T_UINT4; /* unsigned dword */
	    vsize = 4;
	    break;
	case TY_FLOAT:
        last_defined->type = T_REAL32; /* float */
	    vsize = 4;
	    break;
	case TY_QWORD:
        last_defined->type = T_REAL64; /* qword */
	    vsize = 8;
	    break;
	case TY_TBYTE:
        last_defined->type = T_REAL80; /* TBYTE */
	    vsize = 10;
	    break;
	default:
        last_defined->type = T_PVOID; /*label */
	    vsize = 0;
	    break;
    }
...
static void dbgls_emit(int seg, char *buf)
{
   int len ;
   len = *((short*)buf)+2 ;
   if (len % 4) {
   ...
   obj_out(seg,buf,OUT_RAWDATA | len,0,0) ;
}
int dbgls_numeric_leaf(char *buf, unsigned int size)
{
	if (size < LF_NUMERIC) {
		*(short *)buf = size ;
      return 2 ;
...
static void dbgls_createsegs(void) 
{
    char buf[256] ;
    struct Segment *seg ;
    struct Public *loc ;
    int bits =32 ;
    int sig = CV_SIGNATURE_32BIT ;
    int theseg ;
    strcpy(buf,"$$TYPES DWORD CLASS=DEBTYP USE32") ;
    theseg = obj_segment(buf,2,&bits) ;
    obj_out(theseg,&sig,OUT_RAWDATA | 4,0,0) ;
    if (arrhead != NULL) {
        struct Array *arrtmp = arrhead ;
        while (arrtmp) {
            ...
            int len ;
			ptr->leaf = LF_ARRAY ;
            ptr->elemtype = arrtmp->basetype ;
            ptr->idxtype = T_UINT4 ;
            len = dbgls_numeric_leaf(ptr->data,arrtmp->size) ;
            len = sizeof(lfArray)-1 + len ;
            *(short *)buf = len ;
            dbgls_emit(theseg,buf) ;
            arrtmp = arrtmp->next ;
        }
    }
    strcpy(buf,"$$SYMBOLS DWORD CLASS=DEBSYM USE32") ;
    theseg = obj_segment(buf,2,&bits) ;
    obj_out(theseg,&sig,OUT_RAWDATA | 4,0,0) ;
    if (obj_entry_seg != NO_SEG) {
        LABELSYM32 *p = buf ;
        char *name = "start_of_program" ;
        p->reclen = sizeof(LABELSYM32)+ (p->name[0] = strlen(name))- 2 ;
        p->rectyp = S_LABEL32 ;
        p->off = obj_entry_ofs ;
        p->seg = 1 ; /* code seg */
        p->flags.xxx.bAll = 0 ;
    ...
    for (seg = seghead; seg ; seg = seg->next) { /* DAL */
        /* labels this seg */
        if (seg->lochead && seg->lsdebseg == -1) {
            error(ERR_WARNING,"LS Debug - unknown segment class, defaulting to CODE") ;
            seg->lsdebseg = 1 ;
        }
        for (loc = seg->lochead; loc; loc = loc->next) {
            if (loc->type == 0) {
                error(ERR_WARNING,"LS unknown type in debug format, assuming DWORD") ;
                loc->type = T_UINT4 ;
            }
            if (loc->type == T_PVOID) {
                LABELSYM32 *p = buf ;
                p->reclen = sizeof(LABELSYM32)+ (p->name[0] = strlen(loc->name))- 2 ;
                p->rectyp = S_LABEL32 ;
                p->off = loc->offset ;
                p->seg = seg->lsdebseg ;
                p->flags.xxx.bAll = 0 ;
                strcpy(p->name+1,loc->name) ;
            } else  {
                DATASYM32 *p = buf ;
                p->reclen = sizeof(DATASYM32)+ (p->name[0] = strlen(loc->name))- 2 ;
                p->rectyp = loc->global ? S_GDATA32 : S_LDATA32 ;
                p->off = loc->offset ;
                p->seg = seg->lsdebseg ;
                p->typind = loc->type ;
                strcpy(p->name+1,loc->name) ;
...
static struct dfmt *debug_arr[4] = { /* DAL */
	&borland_debug_form,
    &ladsoft_debug_form, /* DAL */
	&null_debug_form,
	NULL
};
